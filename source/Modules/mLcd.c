...
#define k8BitsMode_2Lines   			((unsigned char)(0x3C))
...
void mLcd_Setup(void)
{
	...
	iDio_PinConfig(kPortE,kPin30,kAlternate1);
	iDio_PinConfig(kPortB,kPin0,kAlternate1);
	iDio_PinConfig(kPortB,kPin20,kAlternate1);
	iDio_PinConfig(kPortE,kPin31,kAlternate1);
  
	// Configuration du Port E: DB0 à DB7 et E
  iDio_SetPortDirection(kPortE,kMaskIo19+kMaskIo18+kMaskIo6+kMaskIo3+kMaskIo2+kMaskIo20+kMaskIo21+kMaskIo30+kMaskIo31,kIoOutput);
  
  // Configuration du Port B, RS et R/W
  iDio_SetPortDirection(kPortB,kMaskIo0+kMaskIo20,kIoOutput);	
}
...
static void mLcd_SendLcdData(unsigned char aData)
{
  ...
  iDio_SetPort(kPortB,kMaskIo0,kIoOn);
...
static bool mLcd_ReadLcdBusy(void)
{
 	...
 	// Configuration du Port E: DB0 à DB7 en Input
 	iDio_SetPortDirection(kPortE,kMaskIo19+kMaskIo18+kMaskIo6+kMaskIo3+kMaskIo2+kMaskIo20+kMaskIo21+kMaskIo30,kIoInput);
  ...
  // Bit RW=1, read
  iDio_SetPort(kPortB,kMaskIo20,kIoOn);
	...
	iDio_SetPort(kPortE,kMaskIo31,kIoOn);
 	...
 	aVal=iDio_GetPort(kPortE,kMaskIo30);
 	
 	// Bit E (start data read/write) à 0
 	iDio_SetPort(kPortE,kMaskIo31,kIoOff);
 	
 	// Configuration du Port E: DB0 à DB7 en Input
 	iDio_SetPortDirection(kPortE,kMaskIo19+kMaskIo18+kMaskIo6+kMaskIo3+kMaskIo2+kMaskIo20+kMaskIo21+kMaskIo30,kIoOutput);
