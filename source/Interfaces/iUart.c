...
void iUart_Config(UartEnum aUart)
	...
	UART2->C2 &= (~(UART_C2_TIE_MASK | UART_C2_TCIE_MASK | UART_C2_RIE_MASK
		| UART_C2_ILIE_MASK));
	UART2->C3 &= (~(UART_C3_ORIE_MASK | UART_C3_NEIE_MASK
	...
	// UART Control Register 2 (UARTx_C2)
	// UART Control Register 3 (UARTx_C3)
	UART1->C2 &= (~(UART_C2_TIE_MASK | UART_C2_TCIE_MASK | UART_C2_RIE_MASK
		| UART_C2_ILIE_MASK));
	UART1->C3 &= (~(UART_C3_ORIE_MASK | UART_C3_NEIE_MASK
		| UART_C3_FEIE_MASK | UART_C3_PEIE_MASK));
...
void UART1_IRQHandler(void)
    ...
    // Lecture de la donn�e re�ue
    UART1->S1;
    aVal = UART1->D;

    // On ressort sur le port s�rie 2 les r�ponses du module
    // Attente fin d'envoi
